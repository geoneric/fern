...
public:
        ...
        Result& result)
    {
        ...
        // Cast array of bool to array of uint8.
        Array<uint8_t, 2> inverted_mask_as_ints(extents);
        core::cast(inverted_mask, inverted_mask_as_ints);

        Array<uint8_t, 2> sum_of_weights(extents);
        convolve<DontDivideByWeights>(inverted_mask_as_ints, kernel,
            sum_of_weights);


        // Multiply the values by the sum of weights.
        MaskedRaster<double, 2> multiplied_values(extents,
