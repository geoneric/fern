...
/*!
  ...
  \param     col_source Index of column to calculat value for.
  ...
  \a nr_cols_kernel appropriately. These arguments configure a section of the
  source and kernel for which the \a source has values to use during the
  calculation.
*/
template<
    ...
    size_t nr_cols_kernel)
{
    ...
    for(size_t row = 0; row < nr_rows_kernel; ++row) {
        for(size_t col = 0; col < nr_cols_kernel; ++col) {
            ...
            value +=
                get(kernel, first_row_kernel + row, first_col_kernel + col) *
                get(source, first_row_source + row, first_col_source + col);
            count +=
                get(kernel, first_row_kernel + row, first_col_kernel + col);
        }
    }
    ...
    DestinationImage& destination)
{
    ...
    // Loop over all cells that are situated in the north west corner and for
...
template<
    ...
    class Kernel,
    class DestinationImage>
void convolve_north_east_corner(
    SourceImage const& source,
    Kernel const& kernel,
    DestinationImage& destination)
{
    ...
    // Loop over all cells that are situated in the north east corner and for
    // which some cells are outside of the kernel.
    for(size_t row_source = 0; row_source < radius_; ++row_source) {
...
template<
    ...
    class Kernel,
    class DestinationImage>
void convolve_south_west_corner(
    SourceImage const& source,
    Kernel const& kernel,
    DestinationImage& destination)
{
    ...
    // Loop over all cells that are situated in the south west corner and for
    // which some cells are outside of the kernel.
...
template<
    ...
    class Kernel,
    class DestinationImage>
void convolve_south_east_corner(
    SourceImage const& source,
    Kernel const& kernel,
    DestinationImage& destination)
{
    ...
    // Loop over all cells that are situated in the south east corner and for
    // which some cells are outside of the kernel.
    for(size_t row_source = nr_rows_source - radius_;
            row_source < nr_rows_source; ++row_source) {
    ...
    DestinationImage& destination)
{
    ...
    // We divide the source image in 9 section, which differ with respect to
    // the number of source cells that fall within the kernel neighborhood:
    ...
    // For each section of the source image, we call a seperate function that
    // will handle the section. This code known exactly were the missing cells
    // are located. This save us lots of checks that aren't necessary. For
    // example, when processing the west side of the source image, the logic
    // can assume that the north, east and south side of the kernel are located
    // within the source image. Only at the west side of the kernel some cells
    // are missing.
