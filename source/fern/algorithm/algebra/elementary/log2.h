...
// -----------------------------------------------------------------------------
#pragma once
#include "fern/algorithm/algebra/elementary/detail/log2.h"
...
/*!
    ...
    @brief      Out of domain policy for fern::algorithm::algebra::log2
                algorithm.
...
/*!
    @ingroup    fern_algorithm_algebra_elementary_group
    @brief      For each value in @a value, compute the binary (base 2)
                logarithm of that value, and write each result value to
                @a result.
    @sa         fern::algorithm::log2::OutOfDomainPolicy,
    ...
    - The value types of @a value and @a result must be floating point and the
      same.
    - All input values not smaller than zero are considered within log2's
      domain.
    ...
    Result& result)
{
    ...
    log2::detail::log2<OutOfDomainPolicy>(input_no_data_policy,
    ...
    Result& result)
{
    ...
    OutputNoDataPolicy output_no_data_policy;
    log2<unary::DiscardDomainErrors>(InputNoDataPolicy{{}},
        output_no_data_policy, execution_policy, value, result);
}
