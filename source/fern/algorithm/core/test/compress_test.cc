...
{
    std::vector<int> values = { 1, 2, 3, 4, 5, 99, 6, 99, 9, 99, 10 };
    std::vector<int> result(values.size());
    size_t count{999999};

    {
        fa::core::compress(execution_policy, values, result, count);
        BOOST_CHECK_EQUAL(count, values.size());
        BOOST_CHECK(result == values);
    }
}
