    ...
    std::string const& name)
{
    ...
    ::GDALDataset* dataset = driver.Create(name.c_str(), nr_cols, nr_rows,
        nr_bands, GDALTypeTraits<T>::data_type, options);
...
GDALDataset::GDALDataset(
    ...
    : Dataset(name, open_mode),
      _driver(GetGDALDriverManager()->GetDriverByName(format.c_str())),
      _dataset(nullptr)
...
std::vector<std::string> GDALDataset::feature_names() const
{
    return std::vector<std::string>{Path(this->name()).stem().generic_string()};
}
    ...
    Path const& path) const
{
    ...
    // /<raster>
    return path == Path("/" + Path(this->name()).stem().generic_string());
}
    ...
    Path const& path) const
{
    ...
    // /<raster>/<raster>
    return Path(std::string("/") + Path(this->name()).stem().generic_string() +
        std::string("/") + Path(this->name()).stem().generic_string()) == path;
}
