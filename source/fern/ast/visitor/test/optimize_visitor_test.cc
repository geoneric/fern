...
#include "fern/ast/optimize_visitor.h"
#include "fern/ast/script_visitor.h"
...
BOOST_AUTO_TEST_CASE(remove_temporary_identifier)
{
    ...
    // one expression, are removed.
    {
        ...
        BOOST_CHECK_EQUAL(_script_visitor.script(), fern::String("d = 5\n"));
...
std::cout << _script_visitor.script().encode_in_utf8() << std::endl;
        BOOST_CHECK_EQUAL(_script_visitor.script(), fern::String("e = 5\n"));
        ...
        tree->Accept(_script_visitor);
        std::cout << _script_visitor.script().encode_in_utf8() << std::endl;
        BOOST_CHECK_EQUAL(_script_visitor.script(), fern::String(
            "d = f(5)"));
    }
